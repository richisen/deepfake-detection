...
class Xception(FeatureExtractor):
    def __init__(self):
        ...
        # Initialize basic model without pretrained weights
        self.xception = externals.xception(num_classes=1, pretrained=None)
        self.xception.last_linear = nn.Linear(2048, 1)
        
        # Load our trained weights using proper path construction
        import os
        model_path = os.path.join('weights', 'Xception_DFDC_bestval.pth')
...
class VGG16(FeatureExtractor):
    def __init__(self):
        ...
        # Modify classifier for 2-class output to match the trained weights
        self.vgg.classifier = nn.Sequential(
            nn.Linear(512 * 7 * 7, 512),
            nn.ReLU(True),
            nn.Dropout(0.5),
            nn.Linear(512, 2)  # Changed to 2 outputs to match weights
        )
